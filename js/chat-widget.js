...
function injectChatStyles() {
  ...
  style.textContent = `
    ...
    /* Login-Required Modal (shown to guests who tap the chat bubble) */
    ...
    .chat-login-modal-overlay.active {
      display: flex;
    }
    .chat-login-modal-box {
      background: #1B070A;
    ...
    .chat-login-modal-icon {
      width: 56px;
      height: 56px;
      border-radius: 50%;
      background: rgba(211, 34, 42, 0.2);
      border: 1px solid rgba(211, 34, 42, 0.4);
      color: #F5C842;
      display: flex;
      align-items: center;
      justify-content: center;
    ...
    .chat-login-modal-title {
      font-size: 16.5px;
      font-weight: 700;
      color: #FFFFFF;
      margin-bottom: 8px;
    }
    .chat-login-modal-desc {
      font-size: 13px;
      color: #D1A8AC;
      margin-bottom: 18px;
      line-height: 1.5;
    }
    .chat-login-modal-btn-primary {
      display: block;
      width: 100%;
      padding: 12px;
      background: linear-gradient(135deg, #D3222A, #A51A20);
      ...
      font-size: 14px;
      margin-bottom: 10px;
      box-shadow: 0 4px 15px rgba(211, 34, 42, 0.4);
    }
    .chat-login-modal-btn-secondary {
      width: 100%;
      padding: 10px;
      background: rgba(255, 255, 255, 0.08);
      color: #D1A8AC;
...
function injectChatUI() {
  ...
  container.innerHTML = `
    ...
    <div class="chat-login-modal-overlay" id="chatLoginRequiredModal">
      <div class="chat-login-modal-box">
        <div class="chat-login-modal-icon"><i class="ph ph-lock-key"></i></div>
        <h3 class="chat-login-modal-title">กรุณาเข้าสู่ระบบก่อน</h3>
        <p class="chat-login-modal-desc">คุณต้องเข้าสู่ระบบหรือสมัครสมาชิกก่อน จึงจะสามารถแชทกับแอดมินได้ครับ</p>
        <a href="login.html" class="chat-login-modal-btn-primary"><i class="ph ph-sign-in"></i> เข้าสู่ระบบ / สมัครสมาชิก</a>
        <button type="button" class="chat-login-modal-btn-secondary" id="chatLoginModalCloseBtn">ไว้คราวหลัง</button>
      </div>
    </div>
  `;
  ...
  const loginModalOverlay = document.getElementById('chatLoginRequiredModal');
  document.getElementById('chatLoginModalCloseBtn').addEventListener('click', closeLoginRequiredModal);
  loginModalOverlay.addEventListener('click', (e) => {
    if (e.target === loginModalOverlay) closeLoginRequiredModal();
  });
}

function showLoginRequiredModal() {
  const modal = document.getElementById('chatLoginRequiredModal');
...
// Toggle Chat Window
function toggleChat() {
  // Guests must log in before opening the chat window — the floating
  // bubble stays visible/clickable for everyone, but tapping it prompts
  // login instead of opening the drawer.
  if (!isChatOpen && !currentAuthUser) {
...
function initChatWidget() {
  ...
  onAuthStateChanged(auth, user => {
    ...
    if (user) {
      ...
      // Guests aren't allowed to chat (Firestore rules also reject reads/
      // writes from signed-out users), so no chat doc is created and no
      // message listener is attached for them.
      currentChatId = null;
